...
def create_skew_self_attention_func(
    layout: Tuple[Tuple[int, ...], Tuple[int, ...]],
    window_size: Tuple[int, ...],
    device: str='cuda'
):
    assert len(layout) == 2, "Layout should be consist of shape and stride"
    shape, stride = layout
    window_size = torch.tensor(window_size, device=device)
    assert len(shape) == len(stride) == len(window_size) == 3, "3 dims"
    X, Y, Z = shape
    x = torch.arange(0, X, device=device)
    y = torch.arange(0, Y, device=device)
    z = torch.arange(0, Z, device=device)
